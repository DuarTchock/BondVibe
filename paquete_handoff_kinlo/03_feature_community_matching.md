...
```
Lee 03_feature_community_matching.md en la raíz. Vas a implementar la feature "Community Matching"
...
**Íconos — estilo Notion:** línea fina y redondeada, monocromáticos. `stroke-width` **1.75** (`absoluteStrokeWidth`), `stroke-linecap/linejoin: round`, `fill:none` (salvo corazón lleno en "Me interesa"). Un solo color desde el token de texto — nunca hardcodeado. En React Native usar **lucide-react-native** con `strokeWidth={1.75}` y extender el `CategoryIcon` existente (no crear otro). Set completo y reglas en **`Kinlo Design System.dc.html`** (§03).

> **Fuente de verdad visual:** `Kinlo Design System.dc.html` documenta todos los tokens, tipografías, componentes e íconos. Este §7 es el resumen; ante cualquier duda, el Design System manda.
