/**
 * Smoke test: mounts every screen with safe global mocks and asserts it renders
 * without throwing (catches "white screen" render crashes the bundler can't —
 * undefined components, hook misuse, undefined access in render).
 *
 * It does NOT exercise behaviour; data sources are stubbed to empty/no-op.
 */
...
jest.mock("firebase/firestore", () => {
  ...
  return {
    collection: jest.fn(() => ({})), doc: jest.fn(() => ({})), query: jest.fn(() => ({})),
...
const params = {
  ...
  amount: 1000, price: 0, reservationId: "r1",
...
describe("Screen smoke render", () => {
  it.each(files)("%s renders without crashing", (name) => {
    ...
    const Screen = require(`../${name}`).default;
