// Auto-generated from assets/legal/*.md
// DO NOT EDIT - Edit the .md files instead

export const TERMS_OF_SERVICE = `# Terms of Service

**Last Updated: January 4, 2026**

Welcome to BondVibe! These Terms of Service ("Terms") govern your use of the BondVibe mobile application ("App") operated by BondVibe ("we," "us," or "our").

By using BondVibe, you agree to these Terms. If you do not agree, do not use the App.

## 1. Eligibility

To use BondVibe, you must:

- Be at least **18 years old**
- Have the legal capacity to enter into a binding agreement
- Not be prohibited from using the service under applicable law
- Not have been previously banned from BondVibe

## 2. Account Registration

### 2.1 Account Creation

- You must provide accurate, complete, and current information
- You are responsible for maintaining the security of your account
- You must not share your account credentials with others
- One account per person

### 2.2 Account Verification

- Email verification is required to use the App
- We may require additional verification for certain features (e.g., hosting paid events)

### 2.3 Account Termination

- You may delete your account at any time in Settings
- We may suspend or terminate accounts that violate these Terms

## 3. User Conduct

### 3.1 You Agree To:

- Treat other users with respect and dignity
- Provide accurate information about yourself and events
- Follow all applicable laws and regulations
- Report suspicious or harmful behavior

### 3.2 You Agree NOT To:

- Harass, threaten, intimidate, or harm other users
- Discriminate based on race, gender, religion, sexual orientation, or any protected characteristic
- Create fake profiles or impersonate others
- Post false, misleading, or deceptive content
- Use the App for illegal activities
- Spam other users or promote unrelated products/services
- Attempt to hack, reverse engineer, or compromise the App
- Scrape data or use automated systems to access the App
- Create events for illegal activities

### 3.3 Violations

Violations may result in:

- Content removal
- Account suspension
- Permanent ban
- Legal action where appropriate

## 4. Events

### 4.1 Event Participation

- BondVibe facilitates connections between users for group events
- **BondVibe does not organize, host, or supervise events**
- You attend events at your own risk
- You are responsible for your own safety and conduct at events

### 4.2 For Event Hosts

If you host events, you agree to:

- Provide accurate event information (date, time, location, description)
- Honor the stated event details and pricing
- Treat all attendees fairly and respectfully
- Comply with all local laws and regulations
- Not discriminate against attendees
- Respond to attendee questions in a timely manner
- Cancel events with reasonable notice when necessary

### 4.3 For Event Attendees

If you attend events, you agree to:

- Arrive on time and prepared
- Respect the host and other attendees
- Follow the host's reasonable instructions
- Leave reviews honestly and fairly

### 4.4 Event Cancellation by Hosts

- Hosts may cancel events through the App
- All attendees will be notified automatically
- Refunds will be processed according to our Refund Policy

## 5. Payments & Refunds

### 5.1 Payment Processing

- Payments are processed securely by **Stripe, Inc.**
- By making a purchase, you agree to Stripe's terms of service
- All prices are displayed in Mexican Pesos (MXN) unless otherwise stated

### 5.2 Platform Fees

- BondVibe charges a service fee on paid events
- Fees are clearly displayed before purchase

### 5.3 Refund Policy

| Cancellation Timing              | Refund Amount              |
| -------------------------------- | -------------------------- |
| 7+ days before event             | 100% minus processing fees |
| 3-7 days before event            | 50% minus processing fees  |
| Less than 3 days                 | No refund                  |
| Host cancels event               | 100% minus processing fees |
| Event doesn't occur as described | Contact support for review |

### 5.4 Disputes

- Contact support@bondvibe.com for payment disputes
- We will review and respond within 7 business days
- Chargebacks may result in account suspension

## 6. Content

### 6.1 Your Content

- You retain ownership of content you post (photos, text, etc.)
- By posting content, you grant BondVibe a non-exclusive, worldwide, royalty-free license to use, display, and distribute your content within the App
- You are responsible for ensuring you have rights to content you post

### 6.2 Prohibited Content

Do not post content that:

- Is illegal, harmful, or offensive
- Infringes on others' intellectual property
- Contains nudity or sexual content
- Promotes violence or hatred
- Contains spam or advertising
- Is false or misleading

### 6.3 Content Moderation

- We may remove content that violates these Terms
- We may report illegal content to authorities
- Repeated violations will result in account termination

## 7. Intellectual Property

- BondVibe and its logo are trademarks of BondVibe
- The App and its original content are protected by copyright
- You may not copy, modify, or distribute our intellectual property without permission

## 8. Privacy

Your use of BondVibe is subject to our [Privacy Policy](/privacy), which explains how we collect, use, and protect your information.

## 9. Safety

### 9.1 Your Safety

- Meet in public places for first-time events
- Tell someone where you're going
- Trust your instincts—leave if you feel uncomfortable
- Report concerning behavior immediately

### 9.2 Our Safety Measures

- User reporting system
- Account verification
- Host verification for paid events
- Content moderation

### 9.3 Disclaimer

**BondVibe does not conduct background checks on users.** You are responsible for your own safety when meeting other users.

## 10. Disclaimers

### 10.1 "As Is" Service

BondVibe is provided "AS IS" and "AS AVAILABLE" without warranties of any kind, either express or implied.

### 10.2 No Guarantee

We do not guarantee:

- The App will be uninterrupted or error-free
- Events will meet your expectations
- Other users are who they claim to be
- The quality of any events

## 11. Limitation of Liability

To the maximum extent permitted by law:

- BondVibe shall not be liable for any indirect, incidental, special, consequential, or punitive damages
- BondVibe shall not be liable for any loss or injury arising from events facilitated through the App
- Our total liability shall not exceed the amount you paid to BondVibe in the past 12 months, or \$100 USD, whichever is greater

## 12. Indemnification

You agree to indemnify and hold harmless BondVibe, its officers, directors, employees, and agents from any claims, damages, or expenses arising from:

- Your use of the App
- Your violation of these Terms
- Your violation of any third-party rights
- Events you host or attend

## 13. Dispute Resolution

### 13.1 Informal Resolution

Before filing a legal claim, contact us at support@bondvibe.com. We'll try to resolve disputes informally within 30 days.

### 13.2 Governing Law

These Terms are governed by the laws of Mexico, without regard to conflict of law principles.

### 13.3 Jurisdiction

Any disputes shall be resolved in the courts of Quintana Roo, Mexico.

## 14. Changes to Terms

- We may modify these Terms at any time
- We will notify you of significant changes via the App or email
- Continued use after changes constitutes acceptance
- If you disagree with changes, stop using the App

## 15. Termination

### 15.1 By You

You may stop using BondVibe and delete your account at any time.

### 15.2 By Us

We may suspend or terminate your access if you:

- Violate these Terms
- Engage in harmful behavior
- Use the App for illegal purposes

### 15.3 Effect of Termination

Upon termination:

- Your right to use the App ends immediately
- We may delete your data per our Privacy Policy
- Provisions that should survive (liability, indemnification) will remain in effect

## 16. General Provisions

### 16.1 Entire Agreement

These Terms constitute the entire agreement between you and BondVibe.

### 16.2 Severability

If any provision is found unenforceable, the remaining provisions remain in effect.

### 16.3 No Waiver

Our failure to enforce any right does not waive that right.

### 16.4 Assignment

You may not assign your rights under these Terms. We may assign our rights at any time.

## 17. Contact Us

**For questions about these Terms:**

- **Email:** support@bondvibe.com
- **In-App:** Settings → Help & Support

---

**By using BondVibe, you acknowledge that you have read, understood, and agree to be bound by these Terms of Service.**

© 2026 BondVibe. All rights reserved.
`;

export const PRIVACY_POLICY = `# Privacy Policy

**Last Updated: January 4, 2026**

BondVibe ("we," "us," or "our") operates the BondVibe mobile application. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you use our app.

## 1. Information We Collect

### 1.1 Information You Provide Directly:

- **Account Information:** Email address, full name, age, location
- **Profile Information:** Bio, avatar/photo, personality quiz responses
- **Event Information:** Events you create, join, or attend
- **Communications:** Messages in event group chats, support requests
- **Payment Information:** When you purchase event tickets, payment is processed by Stripe. We do not store your credit card numbers.

### 1.2 Information Collected Automatically:

- **Device Information:** Device type, operating system, unique device identifiers
- **Usage Data:** Features used, events viewed, time spent in app
- **Location Data:** City/region for event matching (we do not track precise GPS location continuously)
- **Push Notification Tokens:** To send you notifications about events and messages

### 1.3 Information from Third Parties:

- **Firebase Authentication:** Login credentials and authentication status
- **Stripe:** Payment confirmation status (not payment details)

## 2. How We Use Your Information

We use your information to:

- **Provide Services:** Create your account, match you with events, enable group chats
- **Personality Matching:** Use your Big Five personality responses to suggest compatible group events
- **Process Payments:** Facilitate ticket purchases through Stripe
- **Communications:** Send push notifications about events, messages, and important updates
- **Safety & Security:** Detect fraud, enforce our terms, protect users
- **Improvements:** Analyze usage patterns to improve the app
- **Legal Compliance:** Comply with applicable laws and regulations

## 3. Information Sharing

**We DO NOT sell your personal information. Ever.**

We may share information with:

| Recipient         | What We Share                        | Why                                             |
| ----------------- | ------------------------------------ | ----------------------------------------------- |
| Other Users       | Name, avatar, bio, personality type  | To facilitate event connections                 |
| Event Hosts       | Your name when you join their event  | To manage event attendance                      |
| Stripe            | Payment details (directly to Stripe) | To process payments securely                    |
| Firebase (Google) | Account data, app usage              | To provide authentication and database services |
| Law Enforcement   | Account information                  | Only when legally required                      |

## 4. Payment Processing & Refunds

- **Payment Processor:** All payments are processed by Stripe, Inc.
- **What We Store:** Transaction IDs, amounts, event associations. NOT credit card numbers.
- **Refund Policy:**
  - 7+ days before event: 100% refund minus processing fees
  - 3-7 days before event: 50% refund minus processing fees
  - Less than 3 days: No refund
  - Host cancellation: 100% refund minus processing fees

## 5. Your Rights & Choices

You have the right to:

- **Access:** View your personal data in Profile settings
- **Correct:** Update your information anytime
- **Delete:** Delete your account (Settings → Delete Account). This removes your data within 30 days.
- **Export:** Request a copy of your data via support@bondvibe.com
- **Opt-Out:** Disable push notifications in your device settings
- **Withdraw Consent:** Stop using the app at any time

### For EU Users (GDPR):

- Right to access, rectification, erasure, and portability
- Right to object to processing
- Right to lodge a complaint with a supervisory authority

### For California Users (CCPA):

- Right to know what personal information is collected
- Right to delete personal information
- Right to opt-out of sale (we don't sell data)
- Right to non-discrimination

## 6. Data Security

We implement industry-standard security measures:

- **Encryption:** All data transmitted via HTTPS/TLS
- **Authentication:** Secure Firebase Authentication
- **Access Control:** Limited employee access to user data
- **Payment Security:** PCI-compliant payment processing via Stripe
- **Regular Audits:** Periodic security reviews

However, no method of transmission over the Internet is 100% secure.

## 7. Data Retention

- **Active Accounts:** Data retained while your account is active
- **Deleted Accounts:** Data deleted within 30 days of account deletion
- **Payment Records:** Retained for 7 years for legal/tax compliance
- **Anonymized Analytics:** May be retained indefinitely

## 8. Children's Privacy

BondVibe is strictly for users 18 years and older. We do not knowingly collect information from anyone under 18. If we discover we have collected data from a minor, we will delete it immediately.

## 9. Location Information

- **What We Collect:** City/region you select for event matching
- **What We DON'T Collect:** Continuous GPS tracking
- **Location Sharing in Chat:** Optional feature you control
- **How to Disable:** Don't share location in chats; we don't track otherwise

## 10. Push Notifications

We send notifications for:

- New messages in event chats
- Event reminders and updates
- When someone joins your event
- Important account updates

**To Opt-Out:** Disable notifications in your device settings.

## 11. Third-Party Services

| Service           | Purpose                           | Privacy Policy                                               |
| ----------------- | --------------------------------- | ------------------------------------------------------------ |
| Firebase (Google) | Authentication, Database, Storage | [Google Privacy Policy](https://policies.google.com/privacy) |
| Stripe            | Payment Processing                | [Stripe Privacy Policy](https://stripe.com/privacy)          |
| Expo              | Push Notifications                | [Expo Privacy Policy](https://expo.dev/privacy)              |

## 12. International Data Transfers

Your information may be transferred to and processed in:

- United States (Firebase, Stripe servers)
- Your local region

We ensure appropriate safeguards for international transfers.

## 13. Changes to This Policy

We may update this Privacy Policy periodically. We will notify you of significant changes via:

- In-app notification
- Email to your registered address

Continued use after changes constitutes acceptance.

## 14. Contact Us

**For privacy questions, data requests, or concerns:**

- **Email:** privacy@bondvibe.com
- **In-App:** Settings → Help & Support
- **Response Time:** Within 30 days

---

**BondVibe is committed to protecting your privacy and being transparent about our data practices.**

© 2026 BondVibe. All rights reserved.
`;
